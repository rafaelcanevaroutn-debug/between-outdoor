...
function getEstadoEfectivo(salida: Salida): string {
  ...
  if (salida.estado === 'activa') {
    ...
    if (salida.fecha_inicio && new Date(salida.fecha_inicio) < now) {
      return 'completada'
    }
  }
...
export default async function SalidasPage(
  ...
  const activas = salidaList
    ...
    .sort((a, b) => {
      ...
      const timeA = a.fecha_inicio ? new Date(a.fecha_inicio).getTime() : Infinity
      const timeB = b.fecha_inicio ? new Date(b.fecha_inicio).getTime() : Infinity
      return timeA - timeB
    })
            ...
            <div className="flex items-center gap-6 text-sm text-[var(--nieve)] font-medium mb-8 drop-shadow-md">
              <div className="flex items-center gap-2">
                ...
                {heroSalida.tipo_viaje === 'salida_recurrente'
                  ...
                  : (heroSalida.fecha_inicio ? fmtFecha(heroSalida.fecha_inicio) : 'Fecha a convenir')}
              </div>
              <div className="flex items-center gap-2">
            ...
            >
              ...
              <div className="relative z-10">
                ...
                <div className="flex items-center gap-2 text-xs font-medium text-white/90 mt-2 drop-shadow-md">
                  ...
                  <span className="shrink-0">{salida.tipo_viaje === 'salida_recurrente' ? (salida.frecuencia ?? 'Recurrente') : (salida.fecha_inicio ? fmtFechaShort(salida.fecha_inicio) : 'A convenir')}</span>
                </div>
              </div>
            </Link>
          ))}
