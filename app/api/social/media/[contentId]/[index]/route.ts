...
export async function GET(
  ...
  const {data: piece, error} = await admin.from('contenido_generado')
  ...
  try {
    ...
    const metadata = await drive.files.get({fileId, fields: 'name,mimeType,size', supportsAllDrives: true})
    ...
    return new NextResponse(Readable.toWeb(media.data as Readable) as BodyInit, {status: 200, headers})
  } catch (mediaError) {
    console.error('[SOCIAL/MEDIA]', mediaError)
