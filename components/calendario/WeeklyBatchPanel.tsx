...
export default function WeeklyBatchPanel({ calendarCode, calendarName, initialRun, hasSalidas, salidas }: WeeklyBatchPanelProps) {
  ...
  return (
    <div className="flex flex-col gap-5">
      {!hasSalidas && (
        <div className="rounded-2xl px-5 py-4 text-[14px] flex items-center gap-3" style={{ backgroundColor: 'rgba(232,180,92,0.08)', border: '1px solid rgba(232,180,92,0.25)', color: '#E8B45C' }}>
      ...
      {showButton && (
        <div className="rounded-2xl px-6 py-8 text-center flex flex-col items-center" style={{ backgroundColor: '#0D130E', border: '1px solid rgba(255,255,255,0.08)' }}>
          <h2 className="text-[22px] font-bold" style={{ color: '#EAF2EC' }}>¡Todo listo para armar tu semana!</h2>
          <p className="text-[14px] mt-2 mb-6 max-w-md" style={{ color: '#9DB0A4' }}>
            La Inteligencia Artificial redactará los textos y unirá las fotos de tus viajes para crear los posteos automáticamente.
          </p>

          {hasMissingPhotos && (
            <div className="mb-6 rounded-xl p-4 text-[14px] flex flex-col gap-2 text-left" style={{ backgroundColor: 'rgba(248,113,113,0.08)', border: '1px solid rgba(248,113,113,0.25)', color: '#f87171' }}>
          ...
          <button
            ...
            className="w-full sm:w-auto flex items-center justify-center gap-2 px-10 py-4 rounded-xl font-bold text-[16px] shadow-[0_8px_20px_-8px_rgba(52,209,126,0.5)] transition-all duration-200 disabled:opacity-50 disabled:shadow-none hover:-translate-y-0.5 active:translate-y-0"
            style={{ background: 'linear-gradient(135deg,#34D17E,#5CE6A0)', color: '#04130A', cursor: triggering || !hasSalidas || hasMissingPhotos ? 'not-allowed' : 'pointer' }}
          >
            <Sparkles className="w-5 h-5" />
            {run?.status === 'completed' || run?.status === 'error' ? 'Volver a generar la semana' : 'Generar mi semana'}
          </button>
          {triggerError && <p className="text-[13px] font-medium mt-3" style={{ color: '#f87171' }}>{triggerError}</p>}
        </div>
      )}

      {running && (
        <div className="flex flex-col gap-4 rounded-2xl px-8 py-10 text-center" style={{ backgroundColor: '#0D130E', border: '1px solid rgba(52,209,126,0.3)', boxShadow: '0 0 40px -10px rgba(52,209,126,0.1)' }}>
          <div className="mx-auto w-14 h-14 relative flex items-center justify-center">
            <svg className="animate-spin h-full w-full absolute inset-0" fill="none" viewBox="0 0 24 24" style={{ color: 'rgba(52,209,126,0.2)' }}>
              <circle className="opacity-100" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="3" />
            </svg>
            <svg className="animate-spin h-full w-full absolute inset-0" fill="none" viewBox="0 0 24 24" style={{ color: '#5CE6A0' }}>
              <path className="opacity-100" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" />
            </svg>
            <Sparkles className="w-5 h-5 absolute" style={{ color: '#5CE6A0' }} />
          </div>
          <div>
            <h3 className="text-[18px] font-bold" style={{ color: '#EAF2EC' }}>
              La IA está creando tu contenido...
            </h3>
            <p className="text-[14px] mt-2 max-w-sm mx-auto" style={{ color: '#9DB0A4' }}>
      ...
      {run?.status === 'error' && (
        <div className="rounded-2xl px-5 py-4 text-[14px] flex items-center gap-3 mt-4" style={{ backgroundColor: 'rgba(248,113,113,0.08)', border: '1px solid rgba(248,113,113,0.25)', color: '#f87171' }}>
