...
const FORMATOS: { value: FormatoCarrusel; label: string; description: string }[] = [
  { value: 'editorial', label: 'Editorial', description: 'Carrusel temÃ¡tico con el pipeline actual.' },
  { value: 'organico', label: 'OrgÃ¡nico', description: 'Frase, datos mÃ­nimos y protagonismo de las fotos.' },
  { value: 'itinerario', label: 'Itinerario', description: 'Una etapa por cada dÃ­a estructurado.' },
  { value: 'ascenso', label: 'Ascenso', description: 'Historia real de una salida pasada.' },
  { value: 'calendario', label: 'Calendario', description: 'Fechas futuras y feriados en una sola guÃ­a.' },
  { value: 'lugar', label: 'Lugar', description: 'Descubrimiento de puntos de interÃ©s verificados.' },
  { value: 'conversacion', label: 'ConversaciÃ³n', description: 'DiÃ¡logo breve con giro visual outdoor.' },
]
...
export default function CarruselFormatPanel({
  ...
  return (
    <div className="flex flex-col gap-3 rounded-xl p-4" style={{ backgroundColor: '#0D130E', border: '1px solid var(--linea)' }}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <label className="flex flex-col gap-1.5 text-xs" style={{ color: 'var(--piedra)' }}>
            ...
            <select value={sourcePastSalidaId} onChange={e => onSourcePastChange(e.target.value)} disabled={disabled} className="px-3 py-2.5 rounded-lg text-sm focus:outline-none" style={{ backgroundColor: 'var(--blanco-piedra)', border: '1px solid var(--linea)', color: 'var(--tinta)' }}>
              ...
              {past.map(item => <option key={item.id} value={item.id}>{item.nombre} Â· {item.fecha_inicio}</option>)}
            </select>
          </label>
          <label className="flex flex-col gap-1.5 text-xs" style={{ color: 'var(--piedra)' }}>
            PrÃ³xima salida equivalente
            <select value={futureRelatedSalidaId} onChange={e => onFutureRelatedChange(e.target.value)} disabled={disabled} className="px-3 py-2.5 rounded-lg text-sm focus:outline-none" style={{ backgroundColor: 'var(--blanco-piedra)', border: '1px solid var(--linea)', color: 'var(--tinta)' }}>
              ...
              {future.map(item => <option key={item.id} value={item.id}>{item.nombre} Â· {item.fecha_inicio}</option>)}
            </select>
          </label>
        </div>
      )}
      ...
      {formato === 'calendario' && (
        <div className="flex flex-col gap-2">
          <p className="text-xs font-medium" style={{ color: '#7DD9A8' }}>Oportunidades de contenido prÃ³ximas</p>
          {calendarOpportunities.length > 0 ? calendarOpportunities.map((opportunity, index) => {
            ...
            return (
              ...
              >
                <span className="block text-sm font-medium" style={{ color: active ? '#7DD9A8' : 'var(--tinta)' }}>{opportunity.title}</span>
                <span className="block mt-1 text-xs" style={{ color: 'var(--piedra)' }}>{opportunity.description}</span>
                <span className="block mt-2 text-xs font-medium" style={{ color: 'var(--cardon)' }}>{active ? 'OpciÃ³n seleccionada' : opportunity.actionLabel}{index === 0 && !selectedCalendarOpportunityId ? ' Â· Recomendada' : ''}</span>
              </button>
            )
          }) : (
            <p className="rounded-lg px-3 py-2.5 text-xs" style={{ backgroundColor: 'var(--blanco-piedra)', border: '1px solid var(--linea)', color: 'var(--piedra)' }}>
              No hay salidas futuras dentro de los prÃ³ximos 60 dÃ­as.
            </p>
          )}
        </div>
      )}
      ...
      {(eligibility.errors.length > 0 || eligibility.warnings.length > 0) && (
        <div className="flex flex-col gap-1.5 rounded-lg px-3 py-2.5" style={{ backgroundColor: eligibility.errors.length > 0 ? 'rgba(239, 68, 68,.06)' : 'rgba(245,158,11,.06)', border: `1px solid ${eligibility.errors.length > 0 ? 'rgba(239, 68, 68,.2)' : 'rgba(245,158,11,.2)'}` }}>
          {eligibility.errors.map(error => <p key={error} className="text-xs" style={{ color: '#f87171' }}>â€¢ {error}</p>)}
          {eligibility.warnings.map(warning => <p key={warning} className="text-xs" style={{ color: '#F59E0B' }}>â€¢ {warning}</p>)}
        </div>
      )}
    </div>
  )
