...
import {OpenAICreativeBudgetExceededError} from '../lib/creative-lab/openai-budget.ts'
...
test('hace una sola crítica y valida el PNG final de todos en modo estricto', async () => {
  let renders = 0
  let critiques = 0
  const strictModes = []
  const persisted = []
  const result = await runCreativeLabBatch(input, {
    ...
    render: async ({html, strictLayout}) => { renders++; strictModes.push(strictLayout); return new TextEncoder().encode(`png:${html}`) },
  ...
  assert.deepEqual(strictModes, [false, true, false, true])
...
test('presupuesto agotado detiene la tanda y no procesa candidatos siguientes', async () => {
  let renders = 0
  await assert.rejects(() => runCreativeLabBatch(input, {
    generate: async () => [{name: 'A', rationale: '', html: 'a'}, {name: 'B', rationale: '', html: 'b'}],
    render: async () => { renders++; return new Uint8Array([1]) },
    critique: async () => { throw new OpenAICreativeBudgetExceededError('tope alcanzado') },
    persist: async candidate => ({id: candidate.name}),
  }), /tope alcanzado/u)
