...
const source = `<style data-template-css>.row{grid-template-columns: 1fr 250px;}.date {color:var(--brand-primary);text-align:right}</style><main>${[1,2,3,4].map(index => `<div><span class="date" data-slot="salida_${index}_fecha"></span></div>`).join('')}</main>`
