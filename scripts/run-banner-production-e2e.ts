...
async function prepareCase() {
  ...
  const [{data: ownerProfile}, {data: brandIdentity}] = await Promise.all([
    ...
    admin.from('brand_identity').select('drive_folder_id,fotos_folder_id,logo_url,color_acento,color_primario,color_secundario,color_texto,color_fondo,font_title,font_body,mati_cliente_id').eq('user_id', salida.user_id).maybeSingle(),
  ])
