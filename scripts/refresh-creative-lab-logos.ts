...
const candidates = {
  ...
  '9d9a5df3-941b-46e9-8d8d-9ddc3d37b26b': {
    ...
    css: '.brand .logo{width:224px!important;height:96px!important;flex:0 0 224px!important;object-fit:contain!important;object-position:left center!important}.content .facts{bottom:145px!important}.slide>.background{height:560px!important}',
    decision: 'experimental',
  ...
  '726bbbcb-0ff8-4160-825f-d9ac68a170ee': {
    ...
    css: '.brand-lockup{width:620px!important}.brand-lockup .logo{width:210px!important;height:72px!important;flex:0 0 210px!important;object-fit:contain!important;object-position:left center!important}.bottom-panel{background:var(--brand-bg)!important;color:var(--brand-text)!important}.bottom-index{display:none!important}',
    decision: 'experimental',
...
if (adminProfilesError) throw new Error(`No se pudo identificar al aprobador: ${adminProfilesError.message}`)
const approvingAdminId = (adminProfiles ?? []).length === 1 ? adminProfiles?.[0]?.id as string : null
const ids = Object.keys(candidates)
const {data: rows, error} = await admin.from('template_library')
  .select('id,template_id,version,piece_type,mold_type,width,height,variant,slots_schema,branding_tokens,html_template,preview_storage_path,stress_tested_at,stress_test_passed')
  .in('id', ids)
...
for (const row of rows ?? []) {
  ...
  if (candidate.decision === 'approved' && row.stress_test_passed !== true) throw new Error(`${row.id}: no se puede aprobar sin estrés previo aprobado`)
  if (candidate.decision === 'approved' && !approvingAdminId) throw new Error(`${row.id}: se necesita un único perfil admin para registrar la aprobación`)
  const newPath = `${row.template_id}/${row.version}-review-v1.png`
  ...
  const keepsPreviousStress = candidate.decision === 'approved'
  const {error: updateError} = await admin.from('template_library').update({
    html_template: html,
    preview_storage_path: newPath,
    status: candidate.decision,
    approved_by: candidate.decision === 'approved' ? approvingAdminId : null,
    approved_at: candidate.decision === 'approved' ? new Date().toISOString() : null,
    stress_tested_at: keepsPreviousStress ? row.stress_tested_at : null,
    stress_test_passed: keepsPreviousStress,
    stress_test_error: keepsPreviousStress ? null : 'Pendiente después de la revisión visual',
  }).eq('id', row.id)
  if (updateError) {
