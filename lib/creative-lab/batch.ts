...
export interface CreativeLabBatchDependencies {
  ...
  render: (input: { contract: CreativeTemplateContract; html: string; mockData: Record<string, string>; branding: CreativeLabBatchInput['branding'] }) => Promise<Uint8Array>
...
export async function runCreativeLabBatch(
  ...
  for (const candidate of candidates) {
    try {
      const initialPng = await dependencies.render({contract: input.contract, html: candidate.html, mockData: input.mockData, branding: input.branding})
      ...
      const finalPng = corrected
        ? await dependencies.render({contract: input.contract, html: finalHtml, mockData: input.mockData, branding: input.branding})
        : initialPng
