...
async function structuredResponse(config: OpenAIConfig, input: unknown[], schemaName: string, schema: Record<string, unknown>): Promise<unknown> {
  configured(config)
  const response = await (config.fetchImpl ?? fetch)('https://api.openai.com/v1/responses', {
    method: 'POST',
    headers: {Authorization: `Bearer ${config.apiKey}`, 'Content-Type': 'application/json'},
    body: JSON.stringify({model: config.model, store: false, input, text: {format: {type: 'json_schema', name: schemaName, strict: true, schema}}}),
...
export async function generateCreativeCandidates(params: {
  ...
  const rules = `Sos diseñador de contenido estático premium. Devolvé HTML/CSS completo y autónomo, sin JavaScript ni red externa. Usá exactamente un <style data-template-css>. Conservá todos los data-slot del contrato. La raíz .slide debe medir el lienzo exacto y usar overflow:hidden. Colores y fuentes sólo mediante los branding_tokens. No incluyas datos reales: sólo placeholders dentro de los slots. Cada candidato debe ser visualmente distinto.`
  ...
  const parsed = await structuredResponse(params.config, [{role: 'system', content: rules}, {role: 'user', content: user}], 'creative_template_candidates', schema) as {candidates?: CreativeCandidate[]}
...
export async function critiqueCreativeCandidate(params: {
  ...
  ]}], 'creative_template_critique', schema) as {verdict?: 'pass' | 'fix'; issues?: string[]; corrected_css?: string}
