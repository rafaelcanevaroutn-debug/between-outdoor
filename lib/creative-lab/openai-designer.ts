...
export async function critiqueCreativeCandidate(params: {
  ...
  const prompt = `Evaluá la captura con esta rúbrica: ${params.rubric}. Revisá jerarquía, legibilidad, contraste, aire, consistencia, composición y cualquier recorte o desborde visible. Verificá especialmente que el logo completo se lea con claridad, no quede reducido a un ícono y tenga contraste con su superficie. El contrato y los slots ya fueron validados; el overflow y el tamaño mínimo del logo se validarán de forma estricta después de tu corrección. Si requiere cambios, devolvé únicamente el CSS completo para reemplazar el bloque style[data-template-css]; no cambies HTML, copy ni slots. Si pasa, corrected_css debe ser una cadena vacía.`
