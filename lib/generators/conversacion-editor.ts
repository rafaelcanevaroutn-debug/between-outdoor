...
const EXTRA_CTA_PATTERN = /coment[aá]|envianos|escribinos|mandanos|link (?:de|en) la bio|te contamos todo|te mandamos|ped[ií] (?:la )?info|mensaje con la palabra/i
const COMMERCIAL_PATTERN = /\busd\b|\bprecio\b|\bcupos?\b|\bincluye\b|\bnivel\s*:|\balojamiento\b|\btransfer|\bseguro\b|\bkit\b|\breserv[aá]|\binscrib|para que te sumes|\bresetear\b|\b(?:una|\d+|dos|tres|cuatro|cinco|seis|siete|ocho|nueve|diez)\s+semanas?\b|\b(?:\d+|dos|tres|cuatro|cinco|seis|siete|ocho|nueve|diez)\s*d[ií]as?\b|\b(?:\d+|dos|tres|cuatro|cinco|seis|siete|ocho|nueve|diez)\s*noches?\b/i
const GENERIC_SPEAKER = /^(?:(?:un|una)\s+)?(?:amig[oa]|persona|viajer[oa]|cliente|gu[ií]a)\s*[a-z0-9]*$/i
const CLICHE_PATTERN = /[eé]pic[oa]|sin aliento|experiencia [uú]nica|paisajes? (?:incre[ií]bles?|impresionantes?)|viv[ií] una?|aventura (?:es|pura)|v(?:ol|uel)[a-záéíóú]* la cabeza|inmensidad|reinicia|recarg|reset|vor[aá]gine|preparate|la mejor (?:vista|experiencia|aventura|forma)/i
...
const ADVERTISING_VOICE_PATTERN = /\bte (?:est[aá] )?espera(?:ndo)?\b|\bven[ií] a\b|\bsumate\b|\bdescubr[ií]\b|\bviv[ií] la\b|\bcomo nunca\b|\bpara arrancar el a[nñ]o distinto\b|la mejor parte de la vida|otro nivel|desped[ií] el a[nñ]o/i
const INVENTED_COMMITMENT_PATTERN = /(?:saqu[eé]|compr[eé]|reserv[eé]|confirm[eé]).*(?:pasaje|lugar|cupo|viaje|fitz|chalt[eé]n)/i
const INVENTED_GROUP_PATTERN = /(?:nos|les) falta (?:uno|una|alguien)|el grupo ya est[aá]|ya somos|queda un lugar|te sumamos al grupo/i
const DESCRIPTION_PROMO_PATTERN = /no es un plan|la escapada que necesit[aá]s|no te quedes con las ganas|se disfruta mejor|ya est[aá] listo para|capital nacional del trekking|la soluci[oó]n|receta para|aventura enorme|sumando excusas|patagonia de verdad|para sumarte/i
...
function exactDateRange(start: string, end: string): string {
  const formatter = new Intl.DateTimeFormat('es-AR', { day: 'numeric', month: 'long', year: 'numeric', timeZone: 'UTC' })
  return `${formatter.format(new Date(`${start}T00:00:00Z`))} al ${formatter.format(new Date(`${end}T00:00:00Z`))}`
}
...
function cleanDescription(value: string, cta: string, destino: string): string {
  const sentences = value
    ...
    .filter(sentence => sentence && !COMMERCIAL_PATTERN.test(sentence) && !CLICHE_PATTERN.test(sentence) && !POETIC_PATTERN.test(sentence) && !HEALTH_PROMISE_PATTERN.test(sentence) && !ADVERTISING_VOICE_PATTERN.test(sentence) && !EXTRA_CTA_PATTERN.test(sentence) && !UNNATURAL_DIALOGUE_PATTERN.test(sentence) && !UNSUPPORTED_SUMMIT_PATTERN.test(sentence) && !DESCRIPTION_PROMO_PATTERN.test(sentence) && !/¿qu[eé] incluye\??/i.test(sentence))
  let result = ''
...
export function editConversationContent(input: ConversationEditorInput): { descripcion: string; cta: string; slides: ConversationSlide[] } {
  ...
  const slides = selected.map((source, index) => {
    ...
    if (POETIC_PATTERN.test(original) || HEALTH_PROMISE_PATTERN.test(original) || OVERDRAMATIC_PATTERN.test(original) || ADVERTISING_VOICE_PATTERN.test(original) || CLICHE_PATTERN.test(original) || INVENTED_COMMITMENT_PATTERN.test(original) || INVENTED_GROUP_PATTERN.test(original) || UNNATURAL_DIALOGUE_PATTERN.test(original) || PERSONIFICATION_PATTERN.test(original) || UNSUPPORTED_SUMMIT_PATTERN.test(original)) {
    ...
    if (/(?:\.{2,}|…)$/.test(slide.texto_principal)) throw new Error(`Conversación: slide ${index + 1} queda gramaticalmente inconcluso`)
    return slide
  })
  ...
  const descriptionBody = cleanDescription(input.descripcion, cta, destino)
  return {
