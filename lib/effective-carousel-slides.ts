...
export function effectiveCarouselSlideCount(slides: SlideCarrusel[] = [], renderedCount = 0): number {
  return Math.max(slides?.length || 0, renderedCount)
}
...
export function effectiveCarouselSlides(slides: SlideCarrusel[] = [], renderedCount = 0): SlideCarrusel[] {
  ...
  const targetCount = effectiveCarouselSlideCount(actualSlides, renderedCount)
  if (targetCount <= actualSlides.length) return actualSlides

  const result = actualSlides.map(slide => ({...slide}))
